export const aofCommandHandlers = {
  // String commands
  ...
  "db.set.sadd": function (data) {
    this.set.sadd(data[0], ...data.slice(1));
  },
  "db.set.srem": function (data) {
    const key = data.key || data[0]; // Handle both formats
    const values = Array.isArray(data.value) ? data.value : [data.value]; // Ensure array format
    this.set.srem(key, ...values);
  },
  ...
  "db.stream.xadd": function (data) {
    ...
    this.stream.xadd(data.key, data.id, ...data.fields.flat());
  },
  ...
  // Bitmap commands
  "db.bitmaps.setBit": function (data) {
    this.bitmap.setBit(data.key, data.offset, data.value);
  },
  "db.bitmaps.bitOp": function (data) {
